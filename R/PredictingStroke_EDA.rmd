...
skewness(stroke$age)
```
Slight skew in glucose level.

```{r}
# let's investigate the numeric variables further
ggplot(stroke, aes(x=age,color=stroke)) + geom_histogram(bins = 20)+ggtitle("Histogram of age and stroke occurance")

ggplot(stroke, aes(x=avg_glucose_level,color=stroke)) + geom_histogram(bins = 20) + ggtitle("Histogram of average glucose level and stroke occurance")

ggplot(stroke, aes(x=bmi,color=stroke)) + geom_histogram(bins = 20) + ggtitle("Histogram of bmi and stroke occurance")
```
# creating a contingency table of categorical variables

```{r}


prop.table(table(stroke$ever_married, stroke$stroke),margin=1)*100
```

```{r}
prop.table(table(stroke$gender, stroke$stroke),margin=1)*100
```

```{r}
prop.table(table(stroke$work_type, stroke$stroke),margin=1)*100
```

```{r}
prop.table(table(stroke$heart_disease, stroke$stroke),margin=1)*100
```

```{r}
prop.table(table(stroke$Residence_type, stroke$stroke),margin=1)*100
```

```{r}
prop.table(table(stroke$hypertension, stroke$stroke),margin=1)*100
```

```{r}
prop.table(table(stroke$smoking_status, stroke$stroke),margin=1)*100
```

```{r}
# convert factors into dummy var
```